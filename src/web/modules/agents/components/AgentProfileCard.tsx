...
export default function AgentProfileCard({ agent, onOpen, index = 0 }: AgentProfileCardProps) {
  return (
    <button
      type="button"
      onClick={onOpen}
    ...
    >
      ...
      <span className="pointer-events-none absolute inset-x-0 bottom-0 h-0.5 origin-left scale-x-0 bg-brand transition-transform duration-200 group-hover:scale-x-100" />
    </button>
  );
}
