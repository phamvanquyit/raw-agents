...
export function CallAgentGroupNode({ data }: NodeProps<CallAgentGroupNodeType>) {
  ...
  return (
    <div
      className={`relative rounded-lg border bg-card min-w-[180px] transition-all duration-200 ${
        data.isAssigned ? "border-chart-2/25" : "border-border opacity-50"
      }`}
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-white/6">
        <div className="w-6 h-6 rounded-[6px] flex items-center justify-center shrink-0 bg-edge-call-agent/12">
          <Bolt width={14} height={14} className="text-edge-call-agent" />
        </div>
        <span className="text-xs font-semibold text-foreground">Call Agent</span>
      ...
      {connected.length > 0 && (
        <div className="flex flex-col gap-0.5 px-1.5 py-1.5">
          {connected.map((ag) => (
            <div key={ag.id} className="flex items-center gap-2 px-2 py-1.5 rounded-md group hover:bg-accent transition-colors">
              <AppLogo size={13} fill="var(--edge-call-agent)" strokeWidth={1} />
              <span className="text-2xs font-medium text-foreground flex-1 whitespace-nowrap">{ag.name}</span>
              <button
                type="button"
      ...
      {available.length > 0 && (
        <div className={`flex flex-col gap-0.5 px-1.5 py-1.5 ${connected.length > 0 ? "border-t border-white/4" : ""}`}>
          {available.map((ag) => (
            ...
            >
              <AppLogo size={13} className="text-muted-foreground" strokeWidth={1} />
              <span className="text-2xs font-medium text-muted-foreground flex-1 whitespace-nowrap">{ag.name}</span>
      ...
      {/* Empty state */}
      {data.agents.length === 0 && <div className="px-3 py-3 text-2xs text-muted-foreground text-center">No agents available</div>}
    </div>
  );
}
