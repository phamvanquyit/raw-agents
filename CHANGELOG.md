...
### Added
- Datatable `query` can sort by `created_at` / `updated_at` (not only schema columns)

### Changed
- Agent datatable tools and `rawagents.datatable.query` prompts include `order_by` examples (`[{key, dir}]`)
